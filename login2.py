...
class Login:
    def __init__(self, root):
     ...
     # ============ist img ===============
        img_path = r"C:/Users/hp/OneDrive/Desktop/Hotel management system/header2.png"

        if os.path.exists(img_path):
            img1 = Image.open(img_path)
            img1 = img1.resize((1350, 700), Image.Resampling.LANCZOS)
            self.photoimg1 = ImageTk.PhotoImage(img1)

            lblimg = Label(self.root, image=self.photoimg1)
            lblimg.place(x=0, y=0, width=1350, height=700)
         # ========== loginpage ==========
            main_frame = Frame(self.root, bd=4, relief=RIDGE)
            main_frame.place(x=400, y=200, width=420, height=320)

            img_path = r"C:/Users/hp/OneDrive/Desktop/Hotel management system/loginicon.jpeg"

        if os.path.exists(img_path):
            img2 = Image.open(img_path)
            img2 = img2.resize((1550, 130), Image.Resampling.LANCZOS)
            self.photoimg2 = ImageTk.PhotoImage(img1)

            lblimg = Label(self.root, image=self.photoimg2)
            lblimg.place(x=400, y=200, width=30, height=30)
